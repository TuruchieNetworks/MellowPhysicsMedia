...
export class GiantBugsShaderMaterials {
  constructor(params,
    mouse) {
    this.params = params;
  ...
  update() {
    ...
    this.shaders.forEach(shader => {
      if (shader) {
        ...
        shader.uniforms.time.value = (Math.sin(this.sineTime) * 0.5) + 0.5 + Math.cos(0.1 + this.sineTime);
        shader.uniforms.explodeIntensity.value = (Math.sin(this.sineTime) * 0.5) + 0.5 + Math.cos(0.1 + this.sineTime);
      }
    });
  }
}
export default GiantBugsShaderMaterials;