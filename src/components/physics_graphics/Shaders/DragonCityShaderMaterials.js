...
export class DragonCityShaderMaterials {
  constructor(params,
    mouse) {
    this.params = params;
    ...
    this.useDarkSpidershader();
    this.useDragonCityTerrainShader();
    this.useFlyingDragonTerrainShader();
    this.updateEvents();
    this.getShaders();
  }

  useDragonCityTerrainShader() {
    this.dragonCityTerrainShader = {
      uniforms: {
      ...
      fragmentShader: `
        ...
        void main() {
          ...
          vec3 spiderColor = mix(colorButter * uvt, colorDust, rayPower);
          // color *= dragonSpider(spiderColor);
          ...
          if (hovered > 0.0) {
            ...
            vec3 colorPallete = mix(color, min(colorBlend, spiderColor), smoothstep(redBlend, blueButter, greenMoney));
    ...
    this.dragonCityTerrainMaterial = new THREE.ShaderMaterial(this.dragonCityTerrainShader);
  }

  useDarkSpidershader() {
    this.DarkSpidershader = {
      uniforms: {
        time: { value: this.time },
        hovered: { value: this.hovered },
        shapeFactor: { value: this.shapeFactor },
      ...
      fragmentShader: `
        ...
        float dancingSpider(vec3 p, vec2 u) {
          ...
          f = cos(a*3.*ks*-1.680)+ (0.412+ sin(a*0.2)) -r;
          // f = abs(cos(a*3.));
          // f = abs(cos(a*2.5))*.5+.3;
          // f = abs(cos(a*12.)*sin(a*3.))*.8+.1;
          // f = smoothstep(-.5,1., cos(a*10.))*0.2+0.5;
    ...
    this.darkSpiderMaterial = new THREE.ShaderMaterial(this.DarkSpidershader);
  }
...
export default DragonCityShaderMaterials;