...
export class TerrainShaderMaterials {
  ...
  useCityTerrainShader() {
    this.cityTerrainShader = {
      ...
      fragmentShader: `
        ...
        float map(vec3 p) {
          ...
          // Use hash function to in<>?.,/troduce randomness for building properties based on the houseNumber
        ...
        void main() {
          ...
          // Normalize mouse position relative to fragCoord and resolution // Either Mouse Setting Works
          // vec2 mouse = (mousePosition * 2.0 - 1.0); // Convert to [-1, 1] range
          // vec2 mouse = vec2(mousePosition.x * 2.0 - 1.0, mousePosition.y * 2.0 - 1.0); // Convert to [-1, 1] range

          // Create a vec2 from the mousePosition x and y values explicitly
          vec2 mouse = (mousePosition * 2.0 - fragCoord) / resolution.y; // Cyclic Mouse

          // Calculate the distance between the mouse and the fragment
    ...
    this.cityTerrainMaterial = new THREE.ShaderMaterial(this.cityTerrainShader);
  }
  ...
  useTunnelTerrainShader() {
    ...
    this.tunnelTerrainMaterial = new THREE.ShaderMaterial(this.tunnelTerrainShader);
  }
    ...
    this.wiredCityTerrainSDFShader = {
      ...
      fragmentShader: `
        ...
        float map(vec3 p) {
          ...
          // return opSmoothUnion(terrain, smin(building, terrainBirds, opSmoothUnion(bird, flock, terrainBirds)), min(flock, terrain));
