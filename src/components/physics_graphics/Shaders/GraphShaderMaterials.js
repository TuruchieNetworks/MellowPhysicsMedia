...
export class GraphShaderMaterials {
  constructor(width = window.innerWidth,
    ...
    this.useStraigthLineShader();
    this.updateEvents();
  }

  useStraigthLineShader() {
    this.straigthLineShader = {
      uniforms: {
      ...
      fragmentShader: `
        ...
        float plot(vec2 u) {    
            return smoothstep(0.02, 0.0, abs(u.y - u.x));
        }
    ...
    this.straigthLineMaterial = new THREE.ShaderMaterial(this.straigthLineShader);
  }
  ...
  handleResize(renderer, width = window.innerWidth, height = window.innerHeight) {
    ...
    if (this.straigthLineShader) this.updateResolution(this.straigthLineShader, width, height);
  }
  
    updateMouseExit() {
    const shaders = [this.straigthLineShader];
  ...
  update() {
    ...
    if (this.straigthLineShader) {
      this.straigthLineShader.uniforms.shapeFactor.value = this.time * Math.sin(0.001 + this.time);
      this.straigthLineShader.uniforms.time.value = (Math.sin(this.time) * 0.5) + 0.5 + Math.cos(0.1 + this.time);
      this.straigthLineShader.uniforms.explodeIntensity.value = (Math.sin(this.time) * 0.5) + 0.5 + Math.cos(0.1 + this.time);
    }
  }
}
export default GraphShaderMaterials;