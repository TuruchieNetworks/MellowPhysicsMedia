...
export class TerrestialMosaicMaterials {
  constructor(params,
    ...
    this.useTerrestialDragonShader();
    this.useTerrestialMosaicShader();
    this.updateEvents();
    this.getShaders();
  }


  useTerrestialDragonShader() {
    this.terrestialDragonShader = {
      uniforms: {
      ...
      fragmentShader: `
        ...
        float sdFlowers(vec3 p) {
          float t = 0.01;
          float u = 0.001 + sineTime;
          float v = 0.001 + (0.1 * sineTime);
          float m = (0.088 + v)+ (sineTime * t);
        ...
        float waterFront(vec3 p) {
          ...
          // f = abs(cos(a*12.)*sin(a*(3. + u)+ sineTime))*.8+.1;
          // f = abs(cos(a*12.)*tan(a*(3. + u)+ sineTime))*.8+.1;
        ...
        float mapDragons(vec3 p) {
          ...
          vec3 waspWiggle = vec3(
            sin(sineTime * 0.8 + waspSeed * 3.0),
            cos(sineTime * 0.6 + waspSeed * 5.0),
            sin(sineTime * 0.4 + waspSeed * 7.0)
          ) * 1.5;

          float waspSize = 3.0 + noise(p.xy * 0.1) * 2.0;
          float waspTail = q.x * sineTime;
          ...
          vec3 dragonWiggle = vec3(
            cos(sineTime * 0.6 + dragonSeed * 10.0),
            sin(sineTime * 0.4 + dragonSeed * 6.0),
            cos(sineTime * 0.5 + dragonSeed * 4.0)
          ) * 1.8;
        
          float dragonTail = sineTime * q.z;
          float dragonSize = S(sineTime * q.z);
        ...
        void main() {
          ...
          for (int i = 0; i < 80; i++) {
            ...
            wiggleCamera(ro, rd, uv * mousePosition, mouse, sineTime);
          ...
          vec3 uvt = vec3(uv.x, uv.y, smoothstep(mouse.x, mouse.y, sineTime));
          ...
          if (hovered > 0.0) {
            ...
            // color *= vec3(dragonSpider(colorButter));
    ...
    this.terrestialDragonMaterial = new THREE.ShaderMaterial(this.terrestialDragonShader);
  }

  useTerrestialMosaicShader() {
    this.terrestialMosaicShader = {
      uniforms: {
        hovered: { value: this.hovered },
        sineTime: { value: this.sineTime },
        shapeFactor: { value: this.shapeFactor },
      ...
      vec3 colorPallete(vec3 p, vec3 color, vec2 u, vec2 m, float shadowIntensity, float rayPower) {
        float redBlend = blendView(p, fract(dragonSpider(p * sineTime)));
        float blueButter = shadowIntensity + rayPower + S(noise(u.xy * 2.0 + sineTime * 0.7)) * shadowIntensity;
        float bt = smoothsineTime(blueButter + sineTime);
        float greenMoney = smoothstep(bt, sin(sineTime + bt), sqrt(sineTime + u.x));
        float rgR = smoothstep(shadowIntensity, (rayPower + S(noise(u.xy * 4.0 + sineTime * 0.3)) ), shadowIntensity); 
        float rgG = sqrt(shadowIntensity + rayPower + S(noise(u.yx * 3.0 + sineTime * 0.5)) * smoothsineTime(shadowIntensity));
        float rgB = smoothsineTime(blueButter + sineTime);
        vec3 colorBlend = vec3(rgR, rgG, rgB);
        vec3 colorDust = vec3(smoothsineTime(redBlend), sineTime + m.x, sqrt(redBlend));
        vec3 colorButter = mix(colorBlend, colorDust, redBlend * blueButter);
      ...
      void main() {
        ...
        vec3 uvt = vec3(uv.x, uv.y, smoothstep(mouse.x, mouse.y, sineTime));
        float rayPower = t * 0.2 * depthFactor;
        vec3 spider = vec3(dragonSpider(color));
        ...
        // // Call your effect function
        color += mix(spider, colorObj, rayPower);
        ...
        if (hovered > 0.0) {
          ...
          color += mix(color, colorObj, sineTime);//fract( * absT); //, sin(dist * (uv.x + sineTime)));
    ...
    this.terrestialMosaicMaterial = new THREE.ShaderMaterial(this.terrestialMosaicShader);
  }
