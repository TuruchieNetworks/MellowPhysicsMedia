...
export class MillWorkShaderMaterials {
  constructor(params, mouse) {
    ...
    this.useVikingMillWorkShader();
    this.useMillWorkFireShader();
    this.useMillWorkLightShader();
    this.useMillWorkBladeShader();

    this.updateEvents();
    this.getShaders();
  }

  useMillWorkBladeShader() {
    this.millworkBladeShader = {
      uniforms: {
      ...
      fragmentShader: `
        ...
        void main(){
          ...
          vec3 shape = r-vec3(((renderer.w*renderer.x)-r), renderer.y, renderer.z)-renderer.z/(sun);
          color = shape;
          // color = sunLight(uv); 
    ...
    this.atlanticGutterMaterial = new THREE.ShaderMaterial(this.millworkBladeShader);
  } 

  useMillWorkFireShader() {
    this.millworkFireShader = {
      uniforms: {
        hovered: { value: this.hovered },
        sineTime: { value: this.sineTime },
        shapeFactor: { value: this.shapeFactor },
      ...
      fragmentShader: `
        ...
        void main(){
          ...
          if (hovered > 0.0) {
            ...
            color += vec3(1.0 - dist, 1.0 - dist, 1.0); // Use original UV-distance-bacoloring
            color = shape; // Sliding movement 
            color *= (0.1371+sh/a)+ (f/0.975) * sin(time + dist * 10.0); // Add a dynamic oscillating effect based on distance and sineTime
            float opacity = smoothstep(0.6, 0.8, 1.0);
            gl_FragColor = vec4(color, opacity); // Default behavior
          }  
          // gl_FragColor = vec4(color, 1.0);
    ...
    this.millworkFireMaterial = new THREE.ShaderMaterial(this.millworkFireShader);
  } 

  useVikingMillWorkShader() {
    this.vikingMillworkShader = {
      uniforms: {
        hovered: { value: this.hovered },
        sineTime: { value: this.sineTime },
        shapeFactor: { value: this.shapeFactor },
    ...
    this.vikingMillWorkMaterial = new THREE.ShaderMaterial(this.vikingMillworkShader);
  }

  useMillWorkLightShader() {
    this.millworkLightShader = {
      uniforms: {
        hovered: { value: this.hovered },
        sineTime: { value: this.sineTime },
        shapeFactor: { value: this.shapeFactor },
    ...
    this.millworkLightMaterial = new THREE.ShaderMaterial(this.millworkLightShader);
  }
  ...
  //           color += vec3(1.0 - dist, 1.0 - dist, 1.0); // Use original UV-distance-bacoloring
  //           color = shape; // Sliding movement 
  //           color *= (0.371)+ (0.5) * sin(time + dist * 10.0); // Add a dynamic oscillating effect based on distance and sineTime
  //           float opacity = smoothstep(0.6, 0.8, 1.0);
  //           gl_FragColor = vec4(color, opacity); // Default behavior
  //         }  
  //         // gl_FragColor = vec4(color, 1.0);
  ...
  //   this.clarkAtlanticMaterial = new THREE.ShaderMaterial(this.millworkLightShader);
  // }
...
export default MillWorkShaderMaterials;